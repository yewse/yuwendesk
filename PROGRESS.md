...
- **T01 外部门**：真实发布公钥/私钥托管与轮换、持有人签名服务、授权分发地址、真实签名安装器、SmartScreen 和干净 Windows 升级仍 `BLOCKED_EXTERNAL`；UPD-001 仅记本地工程证据，UPD-004 保持 `NOT_RUN/BLOCKED_EXTERNAL`。T03–T04 尚未执行。
...
- **T02 外部门/限制**：真实签名安装器、旧程序二进制恢复、干净 Windows 升降级、SmartScreen 与正式分发仍 `BLOCKED_EXTERNAL`。内部 journal/恢复 manifest 显式读取上限及迁移控制文件部分移动专用故障钩子为非阻断 Minor；真实 API 归因、隐私授权/逐次外发许可、教学专业复核和 Office/WPS 状态不变。T03–T04 尚未执行。

## 下一步

见 `HANDOFF.md`。下一步执行 G10-T03 中文路径、缩放与辅助使用；先做自动化 Unicode/长路径/缩放/键盘/焦点矩阵，缺真实 Windows 显示、IME、屏幕阅读器与人工视觉环境的行保持 `BLOCKED_EXTERNAL`。不要重做 G00–G10-T02。扫描件 OCR、真实学生材料授权/逐次外发许可、真实跨机 Windows/DPAPI、开发态 Electron 窗口、G01 目标安装、真实 API/归因质量、教学复核、Office/WPS、签名与公开上传等外部门继续保留。
