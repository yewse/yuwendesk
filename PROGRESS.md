...
- **T01 外部门**：真实发布公钥/私钥托管与轮换、持有人签名服务、授权分发地址、真实签名安装器、SmartScreen 和干净 Windows 升级仍 `BLOCKED_EXTERNAL`；UPD-001 仅记本地工程证据，UPD-004 保持 `NOT_RUN/BLOCKED_EXTERNAL`。T04 尚未执行。
...
- **T02 外部门/限制**：真实签名安装器、旧程序二进制恢复、干净 Windows 升降级、SmartScreen 与正式分发仍 `BLOCKED_EXTERNAL`。内部 journal/恢复 manifest 显式读取上限及迁移控制文件部分移动专用故障钩子为非阻断 Minor；真实 API 归因、隐私授权/逐次外发许可、教学专业复核和 Office/WPS 状态不变。T04 尚未执行。
...
见 `HANDOFF.md`。下一步执行 G10-T04 冷启动、检索、导出性能与压力：固定种子合成 100 plans/5000 source segments，先记录原始基线，再只优化 profiling 证明的热点；Node 指标明确标为 `ENGINEERING_MEASUREMENT`，不得冒充真实 Electron 冷启动或目标 Win11 8GB/SSD P95。不要重做 G00–G10-T03。扫描件 OCR、真实学生材料授权/逐次外发许可、真实跨机 Windows/DPAPI、开发态 Electron 窗口、G01 目标安装、真实 API/归因质量、教学复核、Office/WPS、签名与公开上传等外部门继续保留。
