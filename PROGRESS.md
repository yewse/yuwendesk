...
- **T01/T02 外部门/限制**：本账本结构完整不等于全量验收通过；干净 Windows、授权材料、真实 API/预算、真实学生资料处理与逐次外发授权、Office/WPS、签名/分发和教师专业复核仍为 `BLOCKED_EXTERNAL` 或 `NOT_RUN`。T02 已聚合为 `BLOCKED`；T03/T04 尚未完成，不能称为发布就绪。

## 下一步

见 `HANDOFF.md`。G10 四个本地工程包和 G11-T01/T02 已完成，不重做 G00–G10 或篡改冻结定义。下一内联包为 G11-T03 SBOM、校验和与签名边界；要关闭发布级 G10/G11，仍需锁定 Electron 二进制、干净 Win11 8GB/SSD、正式签名/可信更新身份、Office/WPS、真实 API、学生资料隐私授权/逐次外发许可和教学专业复核。缺输入继续标 `BLOCKED_EXTERNAL`，不得拿 Node 指标替代。
