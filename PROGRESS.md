...
## G12-T07 教师主导业务闭环与真实教材/WPS 验收 — DONE（可用草案；正式发行与教学认证仍阻断）

- [x] **三步 AI 主导入口**：将“备下一课”收敛为“导入资料 → 填写本课必要信息 → AI 生成结果”。主路径只要求年级、课题、课时和逐次外发授权；课程标准/考试说明为可选来源，缺失时只生成明确标注需核实的通用参考，不冒充官方条款。技术状态、UUID、人工填写模型提示词和长 OCR 正文不再占据主流程。
- [x] **真实教材定位与材料边界**：以用户提供的七年级上册教材 PDF 实际运行，修复带空格目录页误命中和正文标题 OCR 粘连，最终精确选中《春》正文 **第 9 页**、2,350 字来源片段。完整授权片段只供计划与内部来源锚点使用，不再重复灌入学生讲义或课堂课件；回归先 RED 后 GREEN。
- [x] **完整业务实跑**：在真实 SQLite/导入/计划/软件审查/工程模拟教师确认链路中完成《春》45 分钟第一课时，导出 PPTX、学生 DOCX/PDF、教师 DOCX/PDF共 5 文件，再修改任务二并重新生成全部 5 文件。计划来源为 `model_assisted_simulated`；真实 DeepSeek 请求没有执行，未冒充真实 API。
- [x] **真实 WPS 执行**：Windows 11 Pro `10.0.26200`、WPS Office Commercial `12.1.0.28022` 中实际打开并导出两份 DOCX 与 PPTX；逐页检查为学生 2 页、教师 2 页、课件 10 页，无裁切。另实际修改 PPTX 首页标题、保存、关闭、重开，10 页和修改标记均保留。WPS PDF 打开、幻灯片放映、打印、长文本/跨页表格和 Microsoft Office 未执行，未提升。
- [x] **当前候选与追加验收**：源码 `d2fe0d8b430325315eecd2fa51feb9cccf94d9ec`；未签名候选 131,051,156 bytes，SHA-256 `39474f3645aec30a72ba91805039757e9ad9b1fc3881860932ca4031b37639d3`。干净运行 `run-20260921-d2fe0d8-04` 绑定同一源码/候选，结果 **12 PASS / 0 FAIL / 32 BLOCKED / 126 NOT_RUN**；新增 WPS PASS 仅为 CLS-002/006/012/013/014/020。
- [x] **验证**：材料回归 17/17；全量 Vitest **707 passed / 1 skipped（708 total，78 files）**；typecheck、lint、候选构建、G12 Electron 纵向（5 文件、一次修改后 5 文件全部变化）、合同校验均通过。SBOM（663 组件）和校验清单通过；`release:verify` 按真实边界退出 2，判定 `BLOCKED / RELEASE_WINDOWS_EVIDENCE_REQUIRED`。
- **教师视角结论**：成品已经是可上手修改的课堂草案，不是大纲或截图；任务链、证据意识、开放答案范围、误解和追问较完整。固定 10 维量表为 **34/40**，但“课程标准证据”和“单元定位”均只有 2/4，触发最低项门槛，结论为 `NEEDS_REVISION`，不能宣称达到特级教师示范课标准。课件首页内部标识与偏朴素的文字版视觉列为已知 P2 观察；正式 P0/P1 缺陷审计仍保持 NOT_RUN，因为它未在候选构建前绑定。
- **外部边界**：网络已恢复，但受保护应用数据库损坏且没有可安全读取的 API 凭据；聊天中的密钥未复制到命令、文件、日志或证据，因此真实 DeepSeek 仍 `BLOCKED_EXTERNAL`。干净标准用户安装、锁定 Node/npm 环境、签名/时间戳、可信分发、合法课标/当地考试原文、本班信息和真人教师复核仍为 `BLOCKED/NOT_RUN`。
- 详细证据：`reports/G12_T07_TEACHER_RUN.md`；用户交付目录：`outputs/YuwenDesk-0.1.0-g12-t07-d2fe0d8`（位于任务输出根，不进入仓库）。

## G13-T01 AI 可用性恢复 — 本地修复完成，真实 API 复验待新候选

- [x] **真实故障定位**：当前生产 `userData/yuwendesk.db` 只读检查返回 `database disk image is malformed`；应用内现存的 `backup_3834a35f-9dc0-4045-8480-e2d99522b90b.ready` 已重新核验哈希、SQLite integrity、schema 和材料目录，备份本身可恢复。该备份按设计不含 API 凭据，也没有模型配置，不能把恢复数据库误写成 AI 已连通。
- [x] **受限本地恢复**：备份页新增“恢复”入口，只接受应用管理目录内的 `backupId`，renderer 不得提交任意路径。恢复前重新验证 manifest、文件哈希、schema、空间与无符号链接，暂存成功后才写 pending marker；启动恢复即使当前数据库已损坏也会先保留其精确字节用于回滚。API 凭据继续排除，恢复后明确要求重新连接 AI。
