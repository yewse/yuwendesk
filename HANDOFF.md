...
- 资料选择页已从“表单 + OCR 全文”改为“先选已导入资料”：每张卡只显示标题、版本、中文资料类型、短摘要和“已选择”，粘贴文字放入折叠备用入口。现有资料仍按当前版本与精确来源范围核验，UI 精简没有放宽来源边界。
- 新备课默认 `AI 生成（推荐）`。AI 页只有一个可选的“补充要求”，留空即可生成完整方案；离线本地自拟仍要求教学重点、核心任务和合理答案范围三项。模型失败只允许显式转本地自拟，不自动重试；切换时清空模型补充要求。
- AI 资料步骤继续要求逐次外发授权，未勾选时按钮禁用并提示可返回本地模式。schema 仅允许三个既有字符串字段为空以支持 AI 主导，类型/长度/额外字段门没有放宽；本地 builder 仍拒绝空教学判断，模型输出仍需严格合同、软件审查和教师确认。
- 新增 6 项 UX/schema 回归均经历 RED→GREEN；全量 **698 passed / 1 skipped（699 total，77 files）**，typecheck、lint 通过。生产构建配合隔离临时 SQLite/合成资料做了实际 Electron 渲染核对；该证据不替代 WPS、安装器、真实 API 或真人专业复核。
- 下一步先提交 T06，再从 clean HEAD 重建固定候选，实际重跑 G12 Electron 纵向、创建新的追加 AcceptanceRun、刷新发行证据/SBOM/最终验证，并生成新的不覆盖旧版本的交付目录。若任何源码或文档在候选后再提交，必须重新绑定整条证据链。
- 真实 DeepSeek、WPS、干净标准用户 Windows、正式签名/时间戳、可信分发、合法现用教材和真人教师专业复核只有实际执行并绑定当前候选后才能转为 `PASS/FAIL`；否则继续 `BLOCKED_EXTERNAL/NOT_RUN`。
...
- 根因不是网络或 `deepseek-flash` 模型名：当前生产 `yuwendesk.db` 已损坏，且应用数据库和系统凭据存储中都没有可用 API 密钥。截图中的预算 `0` 按现有实现表示不限额，也不是阻断原因。
- 已在产品内补上安全的本地备份恢复：设置页只以 `backupId` 恢复应用管理目录内已验证的备份，启动前再次校验 manifest/哈希/integrity/schema/空间，当前数据库即使损坏也先原样进入回滚目录；renderer 不接触文件路径。现有 2026-09-21 备份已只读复核为健康，但未擅自应用到生产数据。
